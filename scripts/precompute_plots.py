...
df = pd.read_parquet(
    os.path.join(ROOT_DIR, "data", "processed", "processed_nlp_features.parquet")
)
...
fig_vol = apply_chart_style(fig_vol)

pio.write_json(
    fig_vol,
    os.path.join(OUTPUT_DIR, "news_volume.json")
)
...
fig_topic_vol = apply_chart_style(fig_topic_vol)

pio.write_json(
    fig_topic_vol,
    os.path.join(OUTPUT_DIR, "topic_volume.json")
)
...
pio.write_json(
    fig_pie,
    os.path.join(OUTPUT_DIR, "sentiment_distribution.json")
)
...
fig_topics = apply_chart_style(fig_topics)

pio.write_json(
    fig_topics,
    os.path.join(OUTPUT_DIR, "topic_distribution.json")
)
...
daily_sentiment['rolling_7d'] = daily_sentiment['sentiment_score'].rolling(7).mean()
...
fig_rolling = px.line(
    ...
    labels={
        'date': 'Date',
        'rolling_7d': '7-Day Rolling Average Sentiment Score'
    }
)

fig_rolling.add_hline(y=0, line_dash="dash", line_color="gray")
fig_rolling = apply_chart_style(fig_rolling)

pio.write_json(
    fig_rolling,
    os.path.join(OUTPUT_DIR, "rolling_sentiment.json")
)

print("Saved rolling_sentiment.json")
...
top_entities = {
    e for e, _ in entity_counter.most_common(80)
}


top_edges = [
    (u, v, w)
    for (u, v), w in edges.most_common(200)
...
print(
    f"Saved entity_network.html ({len(net.nodes)} nodes, {len(net.edges)} edges)"
)


print(
    "\nAll plots precomputed and saved to",
    OUTPUT_DIR
)